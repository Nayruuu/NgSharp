...
// Scriban port of the realistic "devis" document, producing byte-identical output to the NgSharp
// reference (devis.ngsharp.html). Structure is mirrored with {{ for }}/{{ if }}; the number/date/upper/
// largeNumber pipes delegate to the shared Fmt helpers (fr-FR, pinned) so formatting is identical across
// every engine and the benchmark measures only parse + render. Members are accessed PascalCase via the
// same `member => member.Name` renamer used by Engines.Scriban_* so both model members (Issuer.Name) and
// the loop-scoped fields (line.UnitPrice) resolve as-is.
public static class DevisScriban
{
    private static readonly string Source = RealisticEngines.Load("Scriban/devis.scriban");
