...
// Scriban port of the realistic "listes-cartes" (card list) document, producing byte-identical output to
// the NgSharp reference (cartes.ngsharp.html). Structure is mirrored with {{ for }}/{{ if }}; the
...
// (Title, TotalImpressions) and the loop-scoped fields (card.Ref, card.Price) resolve as-is.
public static class CartesScriban
{
    private static readonly string Source = RealisticEngines.Load("Scriban/cartes.scriban");
    ...
    private static string Render(Template template)
    {
        ...
        script.Import(RealisticEngines.CartesModel, renamer: member => member.Name);
