...
// Fluid / Liquid port of the "fiche-produit" (product sheet) document. Produces byte-identical output to the
// NgSharp reference (RealisticEngines.NgSharp_Fiche_*) by delegating ALL number/date/upper/largeNumber
...
public static class FicheFluid
{
    private static readonly string Template = RealisticEngines.Load("Fluid/fiche.fluid.liquid");
    ...
    public static string Cold()
    {
        ...
        return template.Render(new TemplateContext(RealisticEngines.FicheModel, Options));
    }

    // Warm = pre-parsed once (static skeleton), render only each call (matches Engines.Fluid_Warm).
    public static string Warm()
        => Compiled.Render(new TemplateContext(RealisticEngines.FicheModel, Options));
