...
// Fluid / Liquid port of the "devis" document. Produces byte-identical output to the NgSharp reference
// (RealisticEngines.NgSharp_Devis_*) by delegating ALL number/date/upper/largeNumber formatting to the
// shared Fmt helpers through custom Liquid filters — so the engines differ only in parse/render, never in
// formatting. Mirrors Engines.Fluid_Cold / Fluid_Warm (same FluidParser + UnsafeMemberAccessStrategy).
public static class DevisFluid
{
    private static readonly string Template = RealisticEngines.Load("Fluid/devis.fluid.liquid");
