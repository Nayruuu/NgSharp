...
// Handlebars.NET port of the listes-cartes template. Renders byte-identical output to the NgSharp reference
// (RealisticEngines.NgSharp_Cartes_*) so the two engines can be benchmarked on the same document.
//
...
// render, never in formatting. Every conditional here is a plain bool field (Available / Digital / Promo),
// so native {{#if}} already matches NgSharp's [if] bool semantics — no 'truthy' coercion helper needed.
public static class CartesHandlebars
{
    private static readonly string Template = RealisticEngines.Load("Handlebars/cartes.hbs");
    ...
    public static string Cold() => (string)Hb.Compile(Template)(RealisticEngines.CartesModel);

    // Warm: compiled once, invoke only.
    public static string Warm() => (string)Compiled(RealisticEngines.CartesModel);
